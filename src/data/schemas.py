...
class StockContract(Base):
    ...
    # Status and metadata
    status = Column(ChoiceType(ContractStatus), default=ContractStatus.ACTIVE.value, nullable=False)
    created_at = Column(DateTime, default=datetime.datetime.utcnow, nullable=False)
    updated_at = Column(DateTime, default=datetime.datetime.utcnow, onupdate=datetime.datetime.utcnow, nullable=False)
    last_data_update = Column(DateTime, nullable=True)
    
    # Relationships
    market_data_history = relationship("StockMarketData", back_populates="stock_contract", cascade="all, delete-orphan")
...
class OptionContract(Base):
    ...
    # Relationships
    market_data_history = relationship("OptionMarketData", back_populates="option_contract", cascade="all, delete-orphan")
    ...
    __table_args__ = (
        ...
        UniqueConstraint('symbol', 'strike', 'expiration', 'option_type', name='uq_option_contract'),
    ...
    def __repr__(self):
        return f"<OptionContract(symbol='{self.symbol}', strike={self.strike}, expiration='{self.expiration}', type='{self.option_type}')>"
...
class StockMarketData(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    stock_contract_id = Column(Integer, ForeignKey('stock_contracts.id'), nullable=False)
    ...
    def __repr__(self):
        return f"<StockMarketData(contract_id={self.stock_contract_id}, timestamp='{self.timestamp}', close={self.close_price})>"
...
class OptionMarketData(Base):
    ...
    # Relationships
    option_contract = relationship("OptionContract", back_populates="market_data_history")
    
    # Constraints
    __table_args__ = (
        UniqueConstraint('option_contract_id', 'timestamp', name='uq_option_market_data'),
    ...
    def __repr__(self):
        return f"<OptionMarketData(contract_id={self.option_contract_id}, timestamp='{self.timestamp}', last={self.last})>"
...
class Portfolio(Base):
    ...
    performance_snapshots = relationship("PortfolioPerformance", back_populates="portfolio", cascade="all, delete-orphan")
    
    # Constraints
    __table_args__ = (
        CheckConstraint('initial_cash > 0', name='ck_portfolio_initial_cash_positive'),
        CheckConstraint('max_position_size_pct > 0 AND max_position_size_pct <= 1', name='ck_portfolio_max_position_size'),
        CheckConstraint('max_portfolio_leverage > 0', name='ck_portfolio_max_leverage_positive'),
    )
    ...
    def __repr__(self):
        return f"<Portfolio(id='{self.portfolio_id}', name='{self.name}', cash={self.current_cash})>"
...
class Position(Base):
    ...
    # Status
    status = Column(ChoiceType(PositionStatus), default=PositionStatus.OPEN.value, nullable=False)
    ...
    # Constraints
    __table_args__ = (
        CheckConstraint('initial_quantity != 0', name='ck_position_initial_quantity_nonzero'),
    ...
    def __repr__(self):
        return f"<Position(id='{self.position_id}', symbol='{self.contract_symbol}', qty={self.current_quantity}, status='{self.status}')>"
...
class Transaction(Base):
    ...
    def __repr__(self):
        return f"<Transaction(id='{self.transaction_id}', type='{self.transaction_type}', qty={self.quantity}, price={self.price})>"
...
class BacktestRun(Base):
    ...
    def __repr__(self):
        return f"<BacktestRun(id='{self.run_id}', name='{self.name}', status='{self.status}')>"
...
def create_additional_indexes(engine):
    ...
    additional_indexes = [
        "CREATE INDEX IF NOT EXISTS idx_option_contracts_symbol_expiration_strike ON option_contracts(symbol, expiration, strike)",
        "CREATE INDEX IF NOT EXISTS idx_stock_market_data_contract_timestamp_desc ON stock_market_data(stock_contract_id, timestamp DESC)",
        "CREATE INDEX IF NOT EXISTS idx_option_market_data_contract_timestamp_desc ON option_market_data(option_contract_id, timestamp DESC)",
        "CREATE INDEX IF NOT EXISTS idx_positions_portfolio_status ON positions(portfolio_id, status)",
        "CREATE INDEX IF NOT EXISTS idx_transactions_portfolio_timestamp_desc ON transactions(portfolio_id, timestamp DESC)",
        "CREATE INDEX IF NOT EXISTS idx_portfolio_performance_timestamp_desc ON portfolio_performance(portfolio_id, timestamp DESC)",
    ]
