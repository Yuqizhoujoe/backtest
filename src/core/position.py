...
class Position:
    ...
    def side(self) -> PositionSide:
        ...
        if self.current_quantity > 0:
            ...
            return PositionSide.LONG if self.initial_quantity > 0 else PositionSide.SHORT
    ...
    def holding_period(self) -> datetime.timedelta:
        ...
        if self.is_closed and self.transactions:
            ...
            closing_transactions = [t for t in self.transactions 
                                  if (t.quantity < 0 and self.initial_quantity > 0) or 
                                     (t.quantity > 0 and self.initial_quantity < 0)]
           ...
           (self.current_quantity < 0 and new_quantity > 0):
            if abs(new_quantity) > 0:
                raise InsufficientPositionError(
                    f"Transaction would reverse position. Current: {self.current_quantity}, "
                    f"Transaction: {transaction.quantity}, New: {new_quantity}"
                )
        ...
        if (transaction.quantity < 0 and self.current_quantity > 0) or \
           (transaction.quantity > 0 and self.current_quantity < 0):
            ...
            if self.current_quantity > 0:
                ...
                pnl = closed_quantity * (transaction.price - avg_entry) * self.multiplier
            else:
                # Closing short position
                pnl = closed_quantity * (avg_entry - transaction.price) * self.multiplier
    ...
    def update_unrealized_pnl(self, current_price: Optional[Decimal] = None) -> None:
        ...
        if self.current_quantity > 0:
            # Long position
            self.unrealized_pnl = self.current_quantity * (current_price - avg_entry) * self.multiplier
        else:
            # Short position
            self.unrealized_pnl = abs(self.current_quantity) * (avg_entry - current_price) * self.multiplier
        
        self.last_update = datetime.datetime.now()
    
    def close_position(self, exit_price: Decimal, exit_time: datetime.datetime, 
                      commission: Decimal = Decimal('0'), fees: Decimal = Decimal('0')) -> Decimal:
        """
    ...
    def partial_close(self, quantity_to_close: int, exit_price: Decimal, 
                     exit_time: datetime.datetime, commission: Decimal = Decimal('0'), 
    ...
    def calculate_return_pct(self) -> Optional[Decimal]:
        ...
        """
        entry_value = abs(self.initial_quantity) * self.avg_entry_price * self.multiplier
        if entry_value == 0:
            return None
    ...
    def calculate_margin_requirement(self, margin_rate: Decimal = Decimal('0.5')) -> Decimal:
        """
        ...
        if isinstance(self.contract, OptionContract) and self.current_quantity < 0:
            ...
            # This is a simplified calculation - real margin requirements are more complex
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'return_pct': float(self.calculate_return_pct()) if self.calculate_return_pct() else None,
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        summary['initial_margin'] = float(self.initial_margin) if self.initial_margin else None
        summary['maintenance_margin'] = float(self.maintenance_margin) if self.maintenance_margin else None
        return summary