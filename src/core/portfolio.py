...
from .position import Position, Transaction, PositionStatus
from .exceptions import (
    InsufficientFundsError, InsufficientMarginError, ExcessiveRiskError,
    InvalidPositionError, ContractNotFoundError, PortfolioError
)
...
class Portfolio:
    ...
    def __init__(self, initial_cash: Decimal, name: str = "Default Portfolio", 
        ...
        """
        self.portfolio_id = portfolio_id or f"portfolio_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
        self.name = name
    ...
    def add_position(self, contract: Union[OptionContract, StockContract], 
                    quantity: int, entry_price: Decimal, 
                    entry_time: Optional[datetime.datetime] = None,
                    commission: Decimal = Decimal('0'), 
                    fees: Decimal = Decimal('0'),
                    position_id: Optional[str] = None) -> Position:
        """
        ...
        required_cash = self._calculate_required_cash(contract, quantity, entry_price, commission, fees)
        ...
        position = Position(
            ...
            position_id=position_id or f"{contract.contract_id}_{entry_time.strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    def close_position(self, contract_id: str, exit_price: Optional[Decimal] = None,
                      exit_time: Optional[datetime.datetime] = None,
                      commission: Decimal = Decimal('0'), 
                      fees: Decimal = Decimal('0')) -> Optional[Decimal]:
        """
        ...
        closing_transaction = position.transactions[-1]  # Last transaction is the closing one
    ...
    def partial_close_position(self, contract_id: str, quantity_to_close: int,
        ...
        realized_pnl = position.partial_close(quantity_to_close, exit_price, exit_time, commission, fees)
        
        # Update portfolio cash
        closing_transaction = position.transactions[-1]  # Last transaction is the partial closing
        proceeds = closing_transaction.net_amount
        self.cash += proceeds
    ...
    def calculate_var(self, confidence_level: Decimal = Decimal('0.95'),
                     time_horizon_days: int = 1) -> Optional[Decimal]:
        """
    ...
    def calculate_beta(self, market_returns: List[Decimal]) -> Optional[Decimal]:
        ...
        """
        if len(self.daily_returns) != len(market_returns) or len(self.daily_returns) < 10:
            return None
    ...
    def get_portfolio_metrics(self) -> PortfolioMetrics:
        ...
        if self.closed_positions:
            ...
            win_rate = Decimal(len(winning_trades)) / Decimal(len(self.closed_positions)) * 100
            
            if winning_trades and losing_trades:
                avg_win = sum(pos.realized_pnl for pos in winning_trades) / len(winning_trades)
                avg_loss = abs(sum(pos.realized_pnl for pos in losing_trades) / len(losing_trades))
        ...
        if len(self.daily_returns) > 1:
            ...
            if std_dev > 0:
                sharpe_ratio = mean_return / std_dev * Decimal('252').sqrt()  # Annualized
    ...
    def _calculate_required_cash(self, contract: Union[OptionContract, StockContract],
                               quantity: int, price: Decimal, commission: Decimal, fees: Decimal) -> Decimal:
    ...
    def _update_portfolio_metrics(self) -> None:
        ...
        self.margin_used = sum(pos.calculate_margin_requirement() for pos in self.positions.values())
